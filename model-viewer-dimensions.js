...
const checkboxExternal = document.querySelector('#show-dimensions-external');
...
checkboxExternal.addEventListener('click', () => {
    checkbox.click()
});
...
const renderSVG = () => {
    ...
    modelViewer.querySelectorAll('button.dim').forEach(btn => {
        const hotspot = modelViewer.queryHotspot(btn.slot);
        if (hotspot) {
...
modelViewer.addEventListener('load', () => {
    ...
    document.querySelector('[data-comprimento]').textContent = `${(size.z * 100).toFixed(0)} cm`;
    document.querySelector('[data-largura]').textContent = `${(size.x * 100).toFixed(0)} cm`;
    document.querySelector('[data-altura]').textContent = `${(size.y * 100).toFixed(0)} cm`;        
